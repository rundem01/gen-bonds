...
## GenVM constraints found the hard way

Eight traps cost real deploy cycles during this build. All eight are now
rejected by `scripts/check_contract.py` before a contract reaches Studio, and
each one is verified both ways — the gate catches the broken form and passes
the fixed one.

| Trap | Symptom | Fix |
| --- | --- | --- |
| `py-genlayer:test` runtime alias | Schema never loads; nothing below line 1 is read | Use the hashed runtime |
| `import time` at module scope | Schema fails on load | Import inside the non-deterministic block |
| `@staticmethod` in the contract class | Schema fails on load | Move helpers to module level |
| `-> dict` / `-> tuple[...]` returns | Schema fails on load | Return a JSON string |
| `map.get(key, default)` on storage | Runtime failure | `if key in map`, or one-arg `.get()` |
| `gl.message.sender_account` | `AttributeError` after consensus | `sender_address` |
| `prompt_comparative(criteria=...)` | `TypeError` after consensus | `principle=` |
| `TreeMap[Address, <dataclass>]` | Writes fine, **fails on read** | Flat primitive maps |
| `Address` as a public parameter | Bare runtime error at the boundary | Take `str`, convert inside |
| `gl.advanced.run_nondet` | Missing namespace | `gl.vm.run_nondet` |

The dataclass-valued TreeMap is the worst of these, because it is invisible
until an entry exists: a freshly deployed contract reads back fine and only
breaks once real data is written. It is isolated in
`contracts/probe/06_address_dataclass_treemap.py` — 35 lines, one write, one
read, reproduces exactly. Bond storage is now thirteen primitive maps and agent
records six, rather than two dataclasses.
